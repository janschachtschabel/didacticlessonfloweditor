...
export async function generateLearningFlow(userInput: string, apiKey: string, model: string) {
  ...
  try {
    // Generate initial learning flow
    const flowResponse = await client.chat.completions.create({
      model,
      messages: [
        {
          role: "system",
    ...
    if (!flowResponse.choices[0].message.content) {
      throw new Error('No response content received from AI model');
    }

    let flowResult;
    try {
      flowResult = JSON.parse(flowResponse.choices[0].message.content);
    } catch (error) {
    ...
    const enhancedEnvironments = await Promise.all(flowResult.environments.map(async (env: any) => {
      ...
      const enhancementResponse = await client.chat.completions.create({
        model,
        messages: [
          {
            role: "system",
      ...
      let enhancedEnv;
      try {
        enhancedEnv = JSON.parse(enhancementResponse.choices[0].message.content);
      } catch (error) {
